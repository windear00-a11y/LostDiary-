...
export async function GET() {
  try {
    ...
    const energyJar = {
      ...
      resonance:
        reactions?.filter((r) => r.reaction_type === "resonance").length || 0,
    };

    const totalLikes =
      energyJar.hope + energyJar.tear + energyJar.resonance ||
      userStories?.reduce((acc, story) => acc + (story.likes_count || 0), 0) ||
      0;

    // Aggregate Top Resonances (Luminous Lines)
    let totalResonances = 0;
    const shiningMoments: {
      text: string;
      count: number;
      storyTitle: string;
    }[] = [];
