...
export async function GET(req: Request) {
  try {
    ...
    let query = supabase
      ...
      .select(
        "id, title, story_content, pen_name, pen_name_tag, dominant_emotion, likes_count, created_at, inspired_by_story_id, library_echoes(paragraph_index, user_id)",
      )
    ...
    const stories = libraryStoriesRaw.map((story) => {
      ...
      if (story.library_echoes) {
        story.library_echoes.forEach((e: any) => {
          echoMap.set(
            e.paragraph_index,
            (echoMap.get(e.paragraph_index) || 0) + 1,
          );
        });
      }
      ...
      return {
        ...
        echoes: Array.from(echoMap.entries()).map(([k, v]) => ({
          paragraph_index: k,
          count: v,
        })),
      };
    ...
    return NextResponse.json(
      { error: "Failed to fetch the global library feed" },
      { status: 500 },
    );
  }
}
