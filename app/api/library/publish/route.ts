...
export async function POST(req: Request) {
  try {
    ...
    if (chapterError || !chapter) {
      return NextResponse.json(
        { error: "Chapter not found or you do not own it" },
        { status: 404 },
      );
    }
    ...
    if (!sealedContent) {
      const { sealAndGeneralizeStory } =
        await import("@/ai-core/library-engine");
      const sealingResult = await sealAndGeneralizeStory(
        finalTitle,
        finalContent,
      );
      if (sealingResult) {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Please set a Pen Name in your Mirror profile before publishing.",
        },
        { status: 400 },
      );
    }
    ...
    if (insertError) {
      if (insertError.code === "23505") {
        // unique violation
        return NextResponse.json(
          { error: "This chapter has already been published to the library." },
          { status: 400 },
        );
      }
      throw insertError;
    ...
    return NextResponse.json(
      { error: "Failed to publish story to the Global Library" },
      { status: 500 },
    );
  }
}
